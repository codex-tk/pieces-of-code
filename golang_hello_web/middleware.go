...
func parseFromHandler(next HandlerFunc) HandlerFunc {
	return func(c *Context) {
		c.Request.ParseForm()
		//fmt.Println(c.Request.Form, c.Request.PostForm)
...
func parseJsonBodyHandler(next HandlerFunc) HandlerFunc {
	return func(c *Context) {
		...
		fmt.Println("parseJsonBodyHandler", c.Params)
		next(c)
	}
}

func staticFileHandler(next HandlerFunc) HandlerFunc {
	var (
		dir       = http.Dir(".")
